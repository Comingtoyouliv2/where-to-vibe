...
enum BuddyPushToTalkShortcut {
    enum ShortcutOption {
        ...
        var displayText: String {
            ...
            case .shiftFunction:
                return "shift + fn"
            case .controlOption:
                // where-to-vibe: this slot is reused for the
                // ctrl + \ shortcut. We keep the enum case name as
                // `.controlOption` to minimize the diff against upstream,
                // but the actual key combo the user presses is ctrl + \.
                return "ctrl + \\"
            case .shiftControl:
        ...
        var keyCapsuleLabels: [String] {
            ...
            case .shiftFunction:
                return ["shift", "fn"]
            case .controlOption:
                // see note in displayText — this is now ctrl + \
                return ["ctrl", "\\"]
            case .shiftControl:
                return ["shift", "control"]
            case .controlOptionSpace:
        ...
        fileprivate var modifierOnlyFlags: NSEvent.ModifierFlags? {
            ...
            case .controlOption:
                // Was [.control, .option] (modifier-only push-to-talk).
                // For ctrl + \ we want a modifier + KEY pattern, so we
                // return nil here and route through spaceShortcutModifierFlags
                // below (the "space" naming is upstream-historical; we treat
                // it as "modifier + key" in general).
                return nil
            case .shiftControl:
                return [.shift, .control]
        ...
        fileprivate var spaceShortcutModifierFlags: NSEvent.ModifierFlags? {
            ...
            case .shiftFunction:
                return nil
            case .controlOption:
                // ctrl + \ — the key code is `pushToTalkKeyCode` below.
                return [.control]
            case .shiftControl:
                return nil
            case .controlOptionSpace:
    ...
    // where-to-vibe: was 49 (Space) for ctrl+option+space style
    // upstream shortcuts. We now use ctrl + \, so the key code is
    // kVK_ANSI_Backslash (42).
    static let pushToTalkKeyCode: UInt16 = 42 // Backslash (kVK_ANSI_Backslash)
