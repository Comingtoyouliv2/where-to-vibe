...
extension Notification.Name {
    static let whereToVibeDismissPanel = Notification.Name("whereToVibeDismissPanel")
}
...
final class MenuBarPanelManager: NSObject {
    ...
    init(companionManager: CompanionManager) {
        ...
        dismissPanelObserver = NotificationCenter.default.addObserver(
            forName: .whereToVibeDismissPanel,
            object: nil,
            queue: .main
        ) { [weak self] _ in
            self?.hidePanel()
    ...
    private func createStatusItem() {
        ...
        button.image = makeWhereToVibeMenuBarIcon()
        button.image?.isTemplate = true
        button.action = #selector(statusItemClicked)
        button.target = self
    }

    /// Draws the where-to-vibe triangle as a menu bar icon. Uses the same shape
    /// and rotation as the in-app cursor so the menu bar icon matches.
    private func makeWhereToVibeMenuBarIcon() -> NSImage {
