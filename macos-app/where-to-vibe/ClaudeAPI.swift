...
class ClaudeAPI {
    ...
    private func makeAPIRequest() -> URLRequest {
        ...
        // If we're talking directly to api.anthropic.com (no proxy in front),
        // attach the auth + version headers ourselves. When `apiURL` is a
        // Cloudflare Worker (or any other proxy), those headers are added
        // upstream and we just send the JSON body.
        if let host = apiURL.host, host == "api.anthropic.com" {
            if let apiKey = AppBundleConfiguration.stringValue(forKey: "AnthropicAPIKey") {
